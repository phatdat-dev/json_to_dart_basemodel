...
bool isSurrogate(String str, int pos) {
  return 0xd800 <= str.codeUnitAt(pos) && str.codeUnitAt(pos) <= 0xdbff && 0xdc00 <= str.codeUnitAt(pos + 1) && str.codeUnitAt(pos + 1) <= 0xdfff;
}
...
int shouldBreak(int start, List<int> mid, int end) {
  ...
  if (eModifierIndex > 1 && all.sublist(1, eModifierIndex).every((c) => c == Extend) && [Extend, E_Base, E_Base_GAZ].indexOf(start) == -1) {
    return Break;
  }
  ...
  if (rIIndex > 0 && all.sublist(1, rIIndex).every((c) => c == Regional_Indicator) && [Prepend, Regional_Indicator].indexOf(previous) == -1) {
  ...
  // GB10. (E_Base | EBG) Extend* ?	E_Modifier
  final previousNonExtendIndex = all.indexOf(Extend) != -1 ? all.lastIndexOf(Extend) - 1 : all.length - 2;
...
int getGraphemeBreakProperty(int code) {
  ...
  if ((0x0300 <= code && code <= 0x036F) || // Mn [112] COMBINING GRAVE ACCENT..COMBINING LATIN SMALL LETTER X
          ...
          (0x3099 <= code &&
              code <= 0x309A) || // Mn   [2] COMBINING KATAKANA-HIRAGANA VOICED SOUND MARK..COMBINING KATAKANA-HIRAGANA SEMI-VOICED SOUND MARK
          ...
          (0x11438 <= code && code <= 0x1143F) || // Mn   [8] A VOWEL SIGN U..A VOWEL SIGN AI
          (0x11442 <= code && code <= 0x11444) || // Mn   [3] A SIGN VIRAMA..A SIGN ANUSVARA
          0x11446 == code || // Mn       A SIGN NUKTA
  ...
  if (0x0903 == code || // Mc       DEVANAGARI SIGN VISARGA
          ...
          (0x11435 <= code && code <= 0x11437) || // Mc   [3] A VOWEL SIGN AA..A VOWEL SIGN II
          (0x11440 <= code && code <= 0x11441) || // Mc   [2] A VOWEL SIGN O..A VOWEL SIGN AU
          0x11445 == code || // Mc       A SIGN VISARGA
  ...
  if (0x261D == code || // So       WHITE UP POINTING INDEX
          ...
          (0x1F595 <= code &&
              code <= 0x1F596) || // So   [2] REVERSED HAND WITH MIDDLE FINGER EXTENDED..RAISED HAND WITH PART BETWEEN MIDDLE AND RING FINGERS
