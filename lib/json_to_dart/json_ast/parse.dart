...
JSONASTException errorEof(String input, List<dynamic> tokenList, Settings settings) {
  final Loc loc = tokenList.length > 0 ? tokenList[tokenList.length - 1].loc.end : Loc(line: 1, column: 1);
      ...
      case _ObjectState.OPEN_OBJECT:
        {
          if (token.type == TokenType.RIGHT_BRACE) {
            if (settings.loc) {
              final src = settings.source ?? "";
              object.loc = Location.create(startToken.loc!.start.line, startToken.loc!.start.column, startToken.loc!.start.offset,
                  token.loc!.end.line, token.loc!.end.column, token.loc!.end.offset, src);
            }
            ...
            if (property != null) {
              ...
              final msg = unexpectedToken(
                  substring(input, token.loc!.start.offset, token.loc!.end.offset), src, token.loc!.start.line, token.loc!.start.column);
              throw JSONASTException(msg, input, src, token.loc!.start.line, token.loc!.start.column);
            }
          }
          break;
        }
      ...
      case _PropertyState._START_:
        {
          if (token.type == TokenType.STRING) {
            final value = parseString(safeSubstring(input, token.loc!.start.offset + 1, token.loc!.end.offset - 1));
      ...
      case _PropertyState.COLON:
        {
          ...
          if (settings.loc != null) {
            final src = settings.source ?? "";
            property.loc = Location.create(startToken.loc!.start.line, startToken.loc!.start.column, startToken.loc!.start.offset,
                value.value.loc.end.line, value.value.loc.end.column, value.value.loc.end.offset, src);
          }
      ...
      case _ArrayState.OPEN_ARRAY:
        {
          if (token.type == TokenType.RIGHT_BRACKET) {
            if (settings.loc != null) {
              final src = settings.source ?? "";
              array.loc = Location.create(startToken.loc!.start.line, startToken.loc!.start.column, startToken.loc!.start.offset, token.loc!.end.line,
                  token.loc!.end.column, token.loc!.end.offset, src);
            }
...
ValueIndex<LiteralNode>? parseLiteral(String input, List<Token> tokenList, int index, Settings settings) {
  ...
  switch (token.type) {
    case TokenType.STRING:
      {
        value = parseString(safeSubstring(input, token.loc!.start.offset + 1, token.loc!.end.offset - 1));
        break;
      }
...
ValueIndex _parseValue(String input, List<Token> tokenList, int index, Settings settings) {
  ...
  if (value != null) {
    ...
    final src = settings.source ?? "";
    final msg =
        unexpectedToken(substring(input, token.loc!.start.offset, token.loc!.end.offset), src, token.loc!.start.line, token.loc!.start.column);
    throw JSONASTException(msg, input, src, token.loc!.start.line, token.loc!.start.column);
  }
}
...
Node parse(input, settings) {
  ...
  final src = settings.source ?? "";
  final msg = unexpectedToken(substring(input, token.loc!.start.offset, token.loc!.end.offset), src, token.loc!.start.line, token.loc!.start.column);
  throw JSONASTException(msg, input, src, token.loc!.start.line, token.loc!.start.column);
}
