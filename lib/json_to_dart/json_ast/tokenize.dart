...
final Map<String, TokenType> keywordTokensMap = {'true': TokenType.TRUE, 'false': TokenType.FALSE, 'null': TokenType.NULL};
...
enum _NumberState { _START_, MINUS, ZERO, DIGIT, POINT, DIGIT_FRACTION, EXP, EXP_DIGIT_OR_SIGN }
...
class ValueNode extends Node {
  ...
  bool operator ==(dynamic other) => other is ValueNode && type == other.type && loc == other.loc && value == other.value && raw == other.raw;
}
...
class ObjectNode extends Node {
  ...
  bool operator ==(dynamic other) =>
      other is ObjectNode && type == other.type && loc == other.loc && _compareDynamicList(this.children, other.children);
}
...
class ArrayNode extends Node {
  ...
  bool operator ==(dynamic other) =>
      other is ArrayNode && type == other.type && loc == other.loc && _compareDynamicList(this.children, other.children);
}
...
class LiteralNode extends Node {
  ...
  bool operator ==(dynamic other) => other is LiteralNode && type == other.type && loc == other.loc && value == other.value && raw == other.raw;
}
...
class ValueIndex<T> {
  ...
  bool operator ==(dynamic other) => other is ValueIndex<T> && value == other.value && index == other.index;
}
      ...
      case _StringState.START_QUOTE_OR_CHAR:
        {
          if (char == '\\') {
            ...
            index++;
            return Token(TokenType.STRING, line, column + index - startIndex, index, safeSubstring(input, startIndex, index));
          } else {
            // buffer.write(char);
            index++;
          }
...
Token? parseNumber(String input, int index, int line, int column) {
  ...
  if (passedValueIndex > 0) {
    return Token(
        TokenType.NUMBER, line, column + passedValueIndex - startIndex, passedValueIndex, safeSubstring(input, startIndex, passedValueIndex));
  }
...
List<Token> tokenize(String input, Settings settings) {
  ...
  while (index < input.length) {
    ...
    if (token != null) {
      ...
      token.loc = Location.create(line, column, index, token.line, token.column, token.index, src);
      tokens.add(token);
      index = token.index;
      line = token.line;
      column = token.column;
      ...
      final src = settings.source ?? "";
      final msg = unexpectedSymbol(substring(input, index, index + 1), src, line, column);
